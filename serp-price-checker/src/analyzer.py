...
PRICE_EXTRACTION_PROMPT = """Eres un experto en análisis de resultados de búsqueda de Google para productos de electrónica.

PRODUCTO BUSCADO: {query}

RESULTADOS DE GOOGLE (JSON):
{serp_json}

TAREA: Analiza estos resultados de búsqueda y extrae información de precios de productos.
...
3. Solo incluye resultados que sean tiendas vendiendo el producto (no reviews, no comparadores, no noticias)
4. Convierte precios españoles a número: "1.299,00 €" → 1299.00

TIENDAS CONOCIDAS A BUSCAR:
- amazon.es, pccomponentes.com, mediamarkt.es, fnac.es, elcorteingles.es
- worten.es, carrefour.es, apple.com, backmarket.es, phonehouse.es
- También cualquier otra tienda online que venda el producto
...
Si no encuentras productos con precios claros, responde: {{"products": [], "total_found": 0}}"""
