...
with st.sidebar:
    ...
    # Solo mostrar opciones de IA si usamos ZenRows
    if not use_csv:
...
if st.button("🚀 Analizar competencia", type="primary", use_container_width=True):
    ...
    with col2:
        ...
        st.metric("💰 Ranking precio", f"#{analysis.your_price_position} de {total_competitors + 1}")
    ...
    with col3:
        if analysis.competitors:
            ...
            st.metric("📉 vs Más barato", f"{diff:+.2f}€", delta=f"{pct:+.1f}%", delta_color="inverse")
        else:
